...
namespace csharp1
{
    ...
    class MyNote
    {
        ...
        static void Main(string[] args)
        {
            ...
            //只能创建同一个对象
            //1、将构造函数私有化
            //2、提供一个静态方法(在同一个类当中)，返回一个对象
            //3、创建一个单例
            //声明一个初值为null的静态字段，在构造函数中做判断：当只有字段值为null时才创建对象
            //      2.抽象工厂模式
            ...
            //}
            #endregion
            //string pwd = Md5Helper.EncryptString("wo ai ba bao jian");
            ...
            #region 利用工厂封装反射过程
            //可配置可扩展：
            //.config中添加<apSettings>	<add key="key值" value="类型名, dll名">	</appSettings>
            ...
            ***********************************************************/
            ...
            #region SQLite
            //SQLite数据库
            //是一种轻量级的数据库完全配置时小于 400KiB，省略可选功能配置时小于250KiB，
            ...
            //1、获取API
            //  云端托管(CDN)   本地托管(localhost)     Web优化(npm、webpack)
            //2、引用库
            //  引用CSS、JS
            //3、加载模块
            //  API基于dojo开发，使用AMD引入模块的方式(在4.x的版本中只支持这种模式)     require([   ], function(Map, MapView){ //在这里写代码     })
            //  Autocasting:用于将JS对象转换为ArcGIS API for JS的类，而无需require加载这些类-----通过renderer:{symbol:{}}自动实例化SimpleRenderer类
            //
            //Vue项目添加引用
            ...
            //Promise:
            //用于处理异步操作，通过func().then(callback, errorCallback)实现
            //可以使用then()链将多个操作连接在一起
